...
\subsection{Duration and Workload}
	...
	In this sprint we manages a workload of 20.1 hours in avrage pr person each week (161 hours/4 persons/2weeks). We always stibe to reach a workload of 20-24 hours and the group managed this 
	at sprint 4.  
...
\subsection{Sprint backlog}
	...
	In this sprint we added all requirements from the product backlog. Since the group have to 
	finish the implementation in this sprint it ended up with not enough time to implement all
	requirements in the backlog. 

	The requirement FR1.2 was not implemented becuase it had a high cost in terms of implementation
	hours. The group decided to not implement this requirement becuase the value for the customer 
	was to low and the implementation time was to high. The requirement is though partly implemented in 
	the pause functionality. The game state is saved when the user exit the game, but the state will be lost 
	when the player turn off the phone. This will be added to further work.

	When the group designed the game, we wanted to add a notification when a new obstable appeard 
	outside the gamescreen (FR1.13). We wanted to have a pointer on the screen so the player knew 
	where the new notification appeard. This is only partly implemented in terms of sound. There
	is a sound effect when a new obstacle appears, but the player will not get the direction.

	The group added a requirement that should limit the number of powerplant could build during
	a level (FR2.7). The group decided that this was not necessary becuase if the player builds
	to many powerplants, the player will not get to the next level. Bacuse of this, we did not 
	implement this requirment.

	In the project description handed out in the start of this project, there was mentioned that
	it could be added power preservation tips for the user to learn something from the game (FR6.6).
	In the first customer meeting, the customer told the group that this was not very important
	and that this requirement was something that NTNU had mentioned, and not something that
	themselves had required. We will add this to further work and have made some space on the 
	"victory screen" where this can be added if Helgelandskraft should change their mind. In terms
	of working hours, this is a fairly easy implementation. 
