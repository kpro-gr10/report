...
\subsection{Manual Testing}
	...
	\begin{tabular}{| p{3cm} | p{7cm} | p{2cm} |}
		...
		{\bf Test Case} & {\bf Result} & {\bf Pass/Retest} \\ \hline
	  	...
	  	%FT-18 Number of Power Plants & Currently not implemented. &  \\ \hline
...
\subsection{Usability test}
	...
	\subsubsection*{Improvements to be made}

	These are changes and improvements that need to be made to the game as a result of the feedback from the usability test.
	...
	The zoomed out mode was intended for just giving an overview of the game, making the game a little harder considering that the player did not know of all buildings needing power at all time, but if the possibility to build in zoomed-out mode will make the game more playable, then this will be done.
	...
	It is obviously more intuitive to tap the coin as opposed to the building. The coin will be moved closer to the building, so that when tapping the coin, the building will also be tapped.

	That a power line is damaged will be indicated by making a cut in it. It will still be red to attract the attention of the player.

	In order to make it more visible to the player that the power plants can be upgraded, an arrow of some sort will be placed over the building.
...
\subsection{Changes to the requirements}
	...
	The first is FR 1.2 that we changed priority on. BEGRUNNELSE
	...
	we want to have a sound of not, but the visual notification will not be implemented. FJERNET PGA AV DUPLIKAT
