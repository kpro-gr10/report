\section{Sprint 3}
	...
	At the end of the second sprint we had much of the key functionality implemented, however there were still a few key functions that were lacking which prevented the customer from getting a good image of the product. The focus of this sprint will therefore be to implement the last key parts of the game's functionality and get the game into a state where it can be tested properly.
...
\subsection{Duration and Workload}
	...
	participate as much as he should. The 3 other group members worked more than 20 hours. 
...
\subsection{Implementation}
	...
	\subsubsection{Power Lines}
		\begin{itemize}
			\item {\bf Build Power Lines: } It is now possible to build power lines
			between buildings. There is no restrictions of what buildings you can connect, 
			but there need to be a power plant in the connected buildings in order to serve the 
			building with power.
			\item {\bf Remove Power Lines: } It is possible to tap power 
			lines to remove them. The algorithm used is called "point to line segment 
			...
			power line to be in "damaged" mode. 
			\item {\bf More damaged power lines in new levels: } When the user reaches a higher level
			more damaged power lines are now appearing. 
			\item {\bf Color of power line: } A power line is black if there is no power served between
			the buildings. This is either when there is no power plant connected or if the connected power 
			plant cannot serve the buildings. If the line is yellow, it 
			signalize that power is being sent to the building. 
			\item {\bf Selected buildings when building power lines:} When buildings a power line, 
			the user tap on the building he or she wants to connect the line between. When tapping the
			first building, it is outlined with a white circle behind the house, so that the player knows
			that it has been successfully selected. The played can then select a second house to connect
			to the power line to.
		\end{itemize}

	\subsubsection{Other Implementations}
		\begin{itemize}
			\item {\bf Collect Money: } When a building is connected to a power plant, it starts
			...
			increases. 
			\item {\bf Enter next level: } It is now possible to reach the next level by earning the level specific amount of money and completing the level.
			\item {\bf Power plant with Limited Power: } A power plant does not have an infinite amount of
			power. The power plant can be upgraded to serve more power.
			\item {\bf Different Buildings: } When the time loop has started, there is appearing
			different types of buildings. Each building requires a different amount of power and is
			also paying a different amount of money when the building is served with power. 
	...
	\subsection*{Algorithms}
		\begin{itemize}
			...
			Algorithm courses. Breadth First Search (from now on referred to as BFS) traverses a graph 
...
\subsection{Functional Testing}
	...
	Table \ref{table:retestssprint3} shows the test cases that did not pass previous tests and were retested this sprint.
...
\subsection{Group Dynamics}
	...
	problems with the communication, but this is not a problem anymore.The harmony in the group 
	is now great and it helps us keep up the good work and meet our sprint goals. We believe having 
	fun is the key to succeed in this project.
...
\subsection{Sprint Retrospective}
	We are quite pleased with the way we work and none of the group members had anything specific to add or remove in the next sprint.

	We have had one situation where two of the team members worked on the same implementation, 
	but we do not think that will happen again and it has not been a problem up until now. 
	The group needs to ensure good communication within the group as well as give good status
